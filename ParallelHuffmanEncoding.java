    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        for (int i = 0, j = 0, k = constitution.length() / numThreads; i < encodedStr.length; i++, j += k) {
            ...
            if (j + k > constitution.length()) {
                new ParallelHuffmanEncoding().compressString(p, compressedChar, encodedStr[i], constitution.substring(j));
            }
            else {
                new ParallelHuffmanEncoding().compressString(p, compressedChar, encodedStr[i], constitution.substring(j, j + k));
            }
        }
        ...
        System.out.println("Parallel Thread Encoding of file Time: " + ((timeOut * 1e-6) - 5000) + " (ms).");
    }
